import {  Mail } from "lucide-react";

export default function Footer() {
  return (
    <footer className="w-full mt-20 border-t border-slate-700/60 py-10 px-4 text-gray-300">
      <div className="max-w-7xl mx-auto flex flex-col items-center gap-6">
        
        <h2 className="text-xl font-semibold text-emerald-400 tracking-wide">
          React Project Dashboard
        </h2>

        <div className="flex gap-6">
          <a
            href="https://github.com/Abhisheksinghchauhan192"
            target="_blank"
            rel="noopener noreferrer"
            className="text-gray-300 hover:text-emerald-400 transition-colors duration-200"
          >
         <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-github-icon lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
          </a>
    

          <a
            href="https://www.linkedin.com/in/abhisheksinghchauhan786/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-gray-300 hover:text-emerald-400 transition-colors duration-200"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-linkedin-icon lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
           </a>

          <a
            href="mailto:abhic4170@gmail.com"
            className="text-gray-300 hover:text-emerald-400 transition-colors duration-200"
          >
            <Mail className="w-6 h-6 hover:scale-110 transition-transform" />
          </a>
        </div>

        <p className="text-sm text-gray-400 text-center max-w-xl leading-relaxed">
          Crafted with ❤️ using React & TailwindCSS.  
          This dashboard is built to help you track and explore  my projects with ease.
        </p>

        <p className="text-xs text-gray-500">
          © {new Date().getFullYear()} Abhishek Singh Chauhan — All rights reserved.
        </p>
      </div>
    </footer>
  );
}
